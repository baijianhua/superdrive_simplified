...
namespace ConnectTo.Foundation.Core
{
    public abstract class SequencableItem : Item
    {
        ...
        protected abstract Message GetNextMessageImpl();
    }
    ...
    class ListSequencable : SequencableItem
    {
        ...
        internal ListSequencable(string conversationID, List<Item> items)
        {
            Preconditions.Check(items != null);
        ...
        protected override Message GetNextMessageImpl()
        {
            ...
            items.RemoveAll(sendable =>
            {
                //TODO bug 为什么sendable == null?
                return sendable == null || sendable.IsPostCompleted || sendable.TransferState == TransferState.Canceled || sendable.TransferState == TransferState.Error;
            });
