...
namespace ConnectTo.Foundation.Core
{
    ...
    public class Device : ICloneable
    {
        ...
        internal Timer ConnectingOutTimer { get; set; }
        ...
        Timer ConnectingOutSocketTimer { get; set; }
        ...
        public void EnsureOnline()
        {
            ...
            ensureExistTimer = Env.DoLater(
                ...
                , TimeSpan.FromMilliseconds(MAX_ALLOW_NOTSEEN_TIME));
        }
        ...
        public void Connect(double timeoutMilliSeconds = 10000d, Action timeoutAction = null, double? socketTimeoutMilliSeconds = null)
        {
            ...
            socketCreated = false;
            ConnectingOutSocketTimer?.Stop();
            if ((socketTimeoutMilliSeconds ?? timeoutMilliSeconds) != timeoutMilliSeconds)
            {
                ConnectingOutSocketTimer = Env.DoLater(() => 
                {
                    OnConnectintOutTimeout(timeoutAction);
                }, TimeSpan.FromMilliseconds(socketTimeoutMilliSeconds ?? timeoutMilliSeconds));
            }

            //一旦启动连接后，启动一个计时器。如果中间连接成功，则清除这个计时器。
            ConnectingOutTimer?.Stop(); //如果以前曾经尝试连接，忽略以前的结果
            ConnectingOutTimer = Env.DoLater(() =>
            {
                OnConnectintOutTimeout(timeoutAction);
    ...
    internal class ChannelWrapper
    {
        ...
        internal ChannelWrapper(Device device, IChannel channel)
        {
            ...
            System.Threading.Tasks.Task.Run(SendWorker_DoWork, source.Token);
        }
        ...
        private async System.Threading.Tasks.Task SendWorker_DoWork()
        {
                    ...
                    if (sendableList.Count > 0)
                    {
                        ...
                        if (message != null)
                        {
                            ...
                            if (message is ThumbnailResponseMessage)
                            {
                                ...
                                if (trm != null)
                                {
                                    Debug.WriteLine("name=" + trm.Name + " id=" + trm.ID + " len=" + trm.Length);
                                }
                            }
                            if (message is ConversationMessage)
                            {
